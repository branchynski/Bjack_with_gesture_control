 ...
 * Author:         Eryk Rutka 
 * Version:        3.3 (Fixed Output Declarations & Hole Card)
 * Description:    
 * Dynamic card renderer using direct font_rom access. Draws 5 slots for 
 * each Player 0, Player 1, Dealer. Added explicit Card Back rendering.
 ********************************************************************************/
 ...
 module card_generator (
    ...
    logic in_card, is_red, in_text, is_flipped, is_hidden, is_border; 
    ...
    always_comb begin
        ...
        for (int i = 0; i < 5; i++) begin
            ...
            if (active_zone) begin
                ...
                // Border logic for all cards
                if (vga_in.hcount < SLOT_X[i] + 4 || vga_in.hcount >= SLOT_X[i] + CARD_W - 4 ||
                    vga_in.vcount < base_y + 4 || vga_in.vcount >= base_y + CARD_H - 4) begin
                    is_border = 1'b1;
                end
                
                // Hide logic: Dealer's first card
                if (base_y == DEALER_Y && i == 0 && !reveal_dealer) begin
                    is_hidden = 1'b1;
                end else begin
    ...
    localparam DEL_W = 11 + 11 + 12 + 1 + 1 + 1 + 1 + 1 + 1 + 1 + 3 + 1 + 1 + 1; // 47 bits
    logic [DEL_W-1:0] delay_in, delay_out;

    assign delay_in = {
        vga_in.hcount, vga_in.vcount, vga_in.rgb, vga_in.hsync, vga_in.vsync, 
        vga_in.hblnk, vga_in.vblnk, in_card, is_red, in_text, char_x_bit, is_flipped, is_hidden, is_border 
    };

    logic [10:0] d_hcount, d_vcount; logic [11:0] d_rgb;
    logic d_hsync, d_vsync, d_hblnk, d_vblnk, d_in_card, d_is_red, d_in_text, d_is_flipped, d_is_hidden, d_is_border;
    logic [2:0] d_char_x_bit;

    assign {
        d_hcount, d_vcount, d_rgb, d_hsync, d_vsync, d_hblnk, d_vblnk, 
        d_in_card, d_is_red, d_in_text, d_char_x_bit, d_is_flipped, d_is_hidden, d_is_border
    } = delay_out;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (d_in_text) begin
                ...
                if (d_is_hidden) begin
                    out_rgb <= d_is_border ? 12'hf_f_f : 12'h1_2_8; // White frame, Dark blue inside 
                end else begin
                    out_rgb <= 12'hf_f_f; // Regular card
                end
            end else begin
                out_rgb <= d_rgb;
            end
