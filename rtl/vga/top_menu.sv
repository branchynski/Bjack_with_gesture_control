...
module top_menu (
    ...
    /* 2. Instruction: Swipe to select (Bottom left) - Fixed hex literal */
    top_string #(
        .X_POS(50), .Y_POS(730), .ROM_DELAY(2), .TEXT({8'h1A, " SWIPE - SELECT"})
    ) str_inst1 (
        .clk(clk), .rst_n(rst_n), .vga_in(vga_bg), .vga_out(vga_inst1)
    );

    /* 3. Instruction: Knock to enter - Fixed hex literal */
    top_string #(
        .X_POS(250), .Y_POS(730), .ROM_DELAY(2), .TEXT({8'h19, " KNOCK - ENTER"})
    ) str_inst2 (
